const arrayCardio = (function(){
  ...
  //1
  const filter = inventors.filter(person => (person.year >= 1500 && person.year <= 1599));
  console.log(filter);
  //2
  ...
  console.log(sort);
  //4
  const reduce = inventors.reduce((total, person) => {
  ...
  // const de = links.map(link => link.textContent).filter(string => string.includes('de'));
  */
 //7
